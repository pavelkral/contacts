...
AddDialog::AddDialog(const QString title, const QString idgrload,
                     QWidget *parent)
    : QDialog(parent)
...
AddDialog::~AddDialog() {}
void AddDialog::on_btcancel_clicked() { reject(); }
void AddDialog::on_btsave_clicked() {
    ...
    if (title == "New") {
        ...
        if (stname.isEmpty()) {
            ...
            QSqlQuery query("insert into contacts "
                            "(name,phone,mail,adress,notes,idgroup) values ('" +
                            stname + "','" + stphone + "','" + stmail + "','" +
                            stadress + "','" + stnotes + "','" + id_group + "')");
        ...
        QSqlQuery query("update contacts  set name = '" + stname +
                        "' WHERE id = '" + id + "'");
        QSqlQuery query1("update contacts  set mail = '" + stmail +
                         "' WHERE id = '" + id + "'");
        QSqlQuery query2("update contacts  set phone = '" + stphone +
                         "' WHERE id = '" + id + "'");
        QSqlQuery query3("update contacts  set adress = '" + stadress +
                         "' WHERE id = '" + id + "'");
        QSqlQuery query4("update contacts set notes = '" + stnotes +
                         "' WHERE id = '" + id + "'");
        QSqlQuery query5("update contacts set idgroup = '" + id_group +
                         "' WHERE id = '" + id + "'");
